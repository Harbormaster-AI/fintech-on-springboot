...
provider "aws" {
  ...
  default_tags {
    tags = {
      ...
      CertificationId = "0496b587-511e-4fee-974a-456aa69f1fcb"
    }
  }
}
...
resource "aws_key_pair" "generated" {
  key_name   = "pjsk-sshtest-0.1612322336499551"
...
#  name        = "fintechonspringboot-security-group-from-terraform" #optional, when omitted, terraform creates a random name
  description = "security group for application fintechonspringboot created from terraform"
  vpc_id      = aws_vpc.default.id
...
resource "aws_security_group" "db" {
  description = "security group for fintechonspringboot and mysql created from terraform"
  vpc_id      = aws_vpc.default.id
...
#  identifier             = "fintechonspringboot-rds" # Terraform will create a unique id if not assigned
  allocated_storage      = 20
  engine                 = "mysql"
  instance_class         = "db.t3.medium"
  db_name                = "fintechonspringboot"
...
resource "aws_eks_cluster" "this" {
  name     = "eks_cluster_fintechonspringboot"
...
resource "aws_instance" "web" {
  ...
  tags = { Name = "fintechonspringboot instance" } 
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "sudo docker pull theharbormaster/fintech-on-springboot:latest",
      "sudo docker run -p 8000:8000 -p 8080:8080 -e DATABASE_URL=jdbc:mysql://${aws_db_instance.default.endpoint}/fintechonspringboot theharbormaster/fintech-on-springboot:latest"
    ]
  }
}
